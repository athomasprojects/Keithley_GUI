...
class KeithleyGUI:
    def __init__(self, master):
        ...
        self.rear_panel_radio = ttk.Radiobutton(
            self.frame_config,
            text="Rear Panel",
            variable=self.panel_var,
            value=False,
        ...
        self.two_wire_radio = ttk.Radiobutton(
            self.frame_config,
            text="2-wire Mode",
        ...
        self.four_wire_radio = ttk.Radiobutton(
            self.frame_config,
            text="4-wire Mode",
            variable=self.mode_var,
            value=True,
        ...
        # Setup Frame
        self.frame_setup = ttk.LabelFrame(
            master, text="Setup Parameters", padding=(10, 10)
        )
        self.frame_setup.grid(row=3, column=0, padx=10, pady=5, sticky="ew")

        # Setup fields
        self.setup_fields = {
            "Source Type": ttk.Combobox(
                self.frame_setup, values=["VOLT", "CURR"], state="readonly"
            ),
            "Measure Type": ttk.Combobox(
                self.frame_setup, values=["VOLT", "CURR"], state="readonly"
            ),
        ...
        for i, (label, widget) in enumerate(self.setup_fields.items()):
            ttk.Label(self.frame_setup, text=label).grid(
        ...
        ttk.Checkbutton(
            self.log_frame,
            text="Log Scale X",
            variable=self.log_scale_x,
            command=self.update_plot,
        ).pack(side=tk.LEFT)
        ttk.Checkbutton(
            self.log_frame,
            text="Log Scale Y",
            variable=self.log_scale_y,
            command=self.update_plot,
        ).pack(side=tk.LEFT)

        # IV Sweep Button
        self.sweep_button = ttk.Button(
            master, text="Perform IV Sweep", command=self.perform_iv_sweep
        )
        ...
        self.import_button = ttk.Button(
            master, text="Import I-V Data", command=self.import_data
        )
    ...
    def update_button_states(self):
        if self.is_connected:
            ...
            self.save_button.config(
                state="normal" if self.data_collected else "disabled"
            )
    ...
    def connect_instrument(self):
        ...
        try:
            ...
            messagebox.showinfo(
                "Connection", "Successfully connected to the instrument."
            )
    ...
    def async_iv_sweep(
        ...
        try:
            ...
            # Update the plot and collect data if worker has not been signalled to stop
            if not self.stop_event.is_set():
                self.voltage, self.current = v, c
                self.master.after(0, self.update_plot)
                self.data_collected = False  # Data has been collected
            else:
    ...
    def abort_cmd(self):
        ...
        #     return
        if self.is_connected:
            self.instrument.set_return_to_idle_state()
        self.stop_event.set()
    ...
    def update_plot(self):
        ...
        plt.setp(
            self.plot.get_xticklabels(), rotation=45, ha="right", rotation_mode="anchor"
        )
        # Update the plot
        self.canvas.draw()
    ...
    def import_data(self):
        ...
        try:
            with open(filepath, "r") as file:
                reader = csv.reader(
                    file, delimiter="\t" if filepath.endswith(".txt") else ","
                )
